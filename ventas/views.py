...
def nueva_venta(request):
    ...
    if total_pagado < total_final:
        messages.error(
            request,
            f"El total pagado (${total_pagado:,.0f}) no cubre el total de la venta "
...
def detalle_venta_devolucion(request, venta_id):
    ...
    for d in venta.detalles.select_related('producto', 'presentacion').all():
        precio_real = d.precio_unitario * descuento_factor
        subtotal    = precio_real * d.cantidad
        detalles.append({
            'detalle_id':      d.pk,
            ...
            'cantidad':        d.cantidad,
            'precio':          float(round(precio_real, 2)),
